...
class ThermalElements:
    ...
    def load_model(model_name):
        ...
        with open(path, 'r') as text_file:
            ...
            for record in records:
                ...
                if 'cap' in tok0:
                    ...
                    ThermalElements.capacitors.append(capacitor)
                if 'coo' in tok0:
                    cooler = Cooler(tokens[1:6])
                    ThermalElements.capacitors.append(cooler)
                if 'con' in tok0:
                    cap_names = tokens[6].split(';')
                    caps = []
                    for cap_name in cap_names:
                        for cap in ThermalElements.capacitors:
                            if cap.name in cap_name:
                                caps.append(cap)
                                break
                    conductor = Conductor(tokens[1:7], caps)
                    ThermalElements.conductors.append(conductor)
                if 'rad' in tok0:
                    cap_names = tokens[6].split(';')
                    caps = []
                    for cap_name in cap_names:
                        for cap in ThermalElements.capacitors:
                            if cap.name in cap_name:
                                caps.append(cap)
                                break
                    radiator = Radiator(tokens[1:6], caps)
                    ThermalElements.radiators.append(radiator)
        ...
        plot_model = True
        if plot_model:
            ThermalElements.plot_model()
        return

    @staticmethod
    def run():
        delta_time = 1.                         # Model tick in seconds
        run_time = 8*3600.                      # Run for n seconds
        capacitors = ThermalElements.capacitors
        conductors = ThermalElements.conductors
        radiators = ThermalElements.radiators
        ...
        ThermalElements.plot_profiles(capacitors, conductors, radiators, times, temp_series, con_series, rad_series)
        return
    ...
    def plot_model():
        ...
        for capacitor in capacitors:
            ...
            ax.text(xc, yc, text,
                    ...
                    va='center', ha='center')
        for conductor in ThermalElements.conductors:
        ...
        for radiator in ThermalElements.radiators:
    ...
    def plot_profiles(capacitors, conductors, radiators, times, temp_series, con_series, rad_series):
        ...
        for key, capacitor in zip(temp_series, capacitors):
            ...
            ax.plot(times_hr, temperatures, label=label, color=color, ls='solid')
        ...
        for key, conductor in zip(con_series, conductors):
            ...
            ax.plot(times_hr, power_watt, ls=ls_list[0], label=label, color=color)
        for key, radiator in zip(rad_series, radiators):
            ...
            ax.plot(times_hr, power_watt, ls=ls_list[1], label=label, color=color)
